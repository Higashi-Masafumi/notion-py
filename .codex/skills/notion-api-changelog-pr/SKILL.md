...
Track the latest official Notion API changes, classify the impact on `notion-py`, implement only bounded fixes, validate the repo, and publish a draft PR only when the scope and GitHub state are safe.
...
- `auto-fix`: bounded compatibility work that fits existing repo patterns.
...
- add opt-in support for a new Notion API version without flipping `default_notion_version`
...
- Keep breaking-version support opt-in unless the user explicitly asks to flip defaults.
- Update only the minimal set of docs and tests needed to keep examples and compatibility honest.
